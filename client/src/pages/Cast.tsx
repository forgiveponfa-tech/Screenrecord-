...
export default function Cast() {
  ...
  const startBroadcast = async () => {
    ...
    // Pre-check: only hard-block iOS (Apple completely prevents screen capture in all browsers)
    const ua = navigator.userAgent;
    const isIOS = /iPad|iPhone|iPod/.test(ua) && !(window as unknown as Record<string, unknown>).MSStream;

    if (isIOS) {
      setErrorMsg(
        "iPhone and iPad block screen sharing in all browsers — this is an Apple restriction that cannot be bypassed. " +
        "To monitor an iPhone, use Apple's built-in Screen Time app: Settings → Screen Time → App Limits & Downtime."
      );
      setPhase("error");
      return;
    }
    ...
    if (location.protocol !== "https:" && location.hostname !== "localhost") {
      setErrorMsg(
        "Screen sharing requires a secure connection (HTTPS). Make sure you're visiting the https:// version of this site."
      );
      setPhase("error");
      return;
    }
    ...
    try {
      // Get screen capture — let the browser handle what it supports.
      // On Android Chrome this shows a tab/screen picker; on desktop it shows a full screen picker.
      ...
      if (name === "NotAllowedError" || msg.includes("denied") || msg.includes("permission")) {
        setErrorMsg("Screen share permission was denied. Tap Start and then approve the share prompt.");
      } else if (name === "AbortError" || name === "NotReadableError" || msg.includes("cancel") || msg.includes("abort")) {
        setErrorMsg("Screen share was cancelled. Tap Start again and choose a screen or tab to share.");
      } else if (name === "InvalidStateError") {
        setErrorMsg("Please tap Start once and wait for the share prompt to appear.");
      } else if (name === "TypeError" || msg.includes("not a function") || msg.includes("getdisplaymedia")) {
        setErrorMsg(
          "Your browser does not support screen sharing. On Android, open this page in Chrome (version 94 or newer)."
        );
      } else if (name === "NotSupportedError") {
        setErrorMsg(
          "Screen sharing is not supported on this device or browser. Try using Chrome on Android."
        );
      } else {
        setErrorMsg(
          `Screen sharing failed (${name || msg || "unknown error"}). Make sure you are on HTTPS and using Chrome, then try again.`
        );
      }
      setPhase("error");
      cleanup(false);
