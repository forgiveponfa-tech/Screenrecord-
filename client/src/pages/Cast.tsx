...
export default function Cast() {
  ...
  const startBroadcast = async () => {
    ...
    // Pre-check: detect environment and give specific guidance
    ...
    if (!navigator.mediaDevices?.getDisplayMedia) {
      if (isAndroid) {
        setErrorMsg(
          "Please open this page in Chrome for Android (version 94 or newer) to share your screen."
        );
      } else {
        setErrorMsg(
    ...
    try {
      ...
      if (
        ...
        setErrorMsg(
          "Screen sharing is not supported in this browser. Please use Chrome or Edge on a desktop or laptop."
        );
      } else if (name === "InvalidStateError") {
        setErrorMsg("Please tap Start only once and wait for the browser permission prompt.");
      } else {
        setErrorMsg(
          `Could not start screen sharing — ${name ? name : msg || "unknown error"}. Please try again.`
        );
      }
      setPhase("error");
      cleanup(false);
