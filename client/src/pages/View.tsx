...
export default function View() {
  ...
  const loadRecordings = useCallback(async (sessionId: number) => {
    try {
      const recs = await apiFetch<Recording[]>(
  ...
  const connect = useCallback(
    (code: string) => {
      ...
      // Use the shared ICE server list (includes TURN for cross-network support)
      ...
      ws.onmessage = async (event) => {
        ...
        if (msg.type === "offer") {
          ...
          ws.send(
            JSON.stringify({ type: "answer", sessionCode: code, sdp: answer })
          );
      ...
      ws.onerror = () => {
        if (!destroyedRef.current) scheduleRetry(5000);
      };
      ws.onclose = () => {
        if (!destroyedRef.current && connStatus !== "connected")
          scheduleRetry(5000);
      };
    },
        ...
        <div className="w-full max-w-sm space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-gray-400 text-sm">
              Enter the 6-character code from the broadcasting device. After
              this, it auto-connects every time.
            </p>
          </div>
          ...
          <p className="text-center text-xs text-gray-600">
            After connecting once, your code is saved. Just open{" "}
            <span className="font-mono">/view</span> next time — no code needed.
          </p>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#1a1a1a]">
        ...
        <Link href="/dashboard">
          <a className="text-gray-500 hover:text-white text-xs font-mono transition-colors">
            RECORDINGS
          </a>
        </Link>
      </div>
      ...
      <div className="flex-1 flex flex-col px-4 py-4 gap-4 max-w-2xl mx-auto w-full">
              ...
              {connStatus === "waiting" ? (
                <>
                  ...
                  <p className="text-xs text-gray-600">
                    Will connect automatically when they start
                  </p>
                </>
              ) : (
                <>
                ...
                >
                  <div>
                    <p className="text-xs text-gray-400 font-mono">
                      Clip {i + 1} · {formatDuration(rec.durationSeconds)} ·{" "}
                  ...
                  <button
                    ...
                    disabled={loadingPlay && playingId === rec.id}
                    className="text-xs text-green-400 hover:text-green-300 font-mono transition-colors disabled:opacity-50"
                  >
                    {loadingPlay && playingId === rec.id ? "..." : "▶ PLAY"}
                  </button>
                </div>
              ))}
            </div>
        ...
        {/* No recordings yet */}
        {session && recordings.length === 0 && connStatus === "connected" && (
          <p className="text-center text-xs text-gray-600 font-mono">
            No clips saved yet — recordings appear every 10 seconds
          </p>
        )}
      </div>
    </div>
